						  ...
						  ptrdiff_t top_index, ptrdiff_t hole_index, c_compare comp)
{
	...
	__first->alloc_and_copy(&__tmp_parent, __first);
	__first->alloc_and_copy(&__tmp_hole, __first);
	__tmp_parent->advance(__tmp_parent, parent_index);
	__tmp_hole->advance(__tmp_hole, hole_index);
	while (hole_index > top_index) {
		if (comp(C_ITER_DEREF(__tmp_parent), value)) {
			...
			__tmp_parent->assign(__tmp_parent, __first);
			__tmp_parent->advance(__tmp_parent, parent_index);
			__tmp_hole->assign(__tmp_hole, __first);
			__tmp_hole->advance(__tmp_hole, hole_index);
		}
		else
			break;
