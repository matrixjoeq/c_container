...
void c_vector_reserve(c_vector_t* vector, size_t new_cap)
{
	...
	size_t new_size = new_cap * type_info->size();
	c_ref_t start = malloc(new_size);
	if (!start) return;
	...
	vector->end_of_storage = start + new_size;
}
...
c_vector_iterator_t c_vector_insert(c_vector_t* vector, c_vector_iterator_t pos, const c_ref_t data)
{
	...
	assert(is_valid_pos(vector, pos.pos));

	if (vector->end_of_storage == vector->finish) {
	...
	memmove(pos.pos + type_info->size(), pos.pos, vector->finish - pos.pos);
	type_info->copy(pos.pos, data);
...
c_vector_iterator_t c_vector_erase(c_vector_t* vector, c_vector_iterator_t pos)
{
	...
	return create_iterator(type_info, pos.pos);
}
	...
	c_vector_t* vector, c_vector_iterator_t first, c_vector_iterator_t last)
{
	...
	assert(is_valid_pos(vector, first.pos));
	assert(is_valid_pos(vector, last.pos));
	...
	return create_iterator(type_info, first.pos);
}
...
void c_vector_resize_with_value(c_vector_t* vector, size_t count, const c_ref_t data)
{
	...
	assert(type_info->size);
	size_t data_size = type_info->size();
    c_ref_t pos = vector->start;
    while (pos != vector->finish && count > 0) {
        pos += data_size;
        --count;
    }
