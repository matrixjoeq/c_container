...
__c_static c_iterator_t* increment(c_iterator_t* iter)
{
...
__c_static c_iterator_t* decrement(c_iterator_t* iter)
{
	c_list_iterator_t* _iter = (c_list_iterator_t*)iter;
...
__c_static c_list_node_t* create_node(
	c_containable_t* type_info, c_list_node_t** node, c_ref_t data)
{
	assert(node);
	assert(*node == 0);
...
__c_static c_list_node_t* pop_node(c_containable_t* type_info, c_list_node_t* node)
{
	assert(type_info);
...
c_list_iterator_t c_list_insert(c_list_t* list, c_list_iterator_t pos, c_ref_t data)
{
    ...
    if (!create_node(&(list->type_info), &node, data)) {
        return pos;
    }
...
{
	assert(list);
	c_list_node_t* node = pop_node(&(list->type_info), pos.node);	
    return create_iterator(&(list->type_info), node);
}
...
void c_list_pop_back(c_list_t* list)
{
	if (!c_list_empty(list))
		pop_node(&(list->type_info), list->node->prev);
}
...
{
	if (!c_list_empty(list))
		pop_node(&(list->type_info), list->node->next);
}
